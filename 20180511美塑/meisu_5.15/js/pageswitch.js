		...
		PageSwitch.prototype = {
			...
			_scrollPage : function(init){
				...
				if(_prefix){
					...
					//页面滚动的动画 
					window._animateName = "fadeIn fadeInRight fadeInLeft fadeInUp fadeInDown zoomIn hidden scaleIn rotate rotateInDownLeft bounceInDown rubberBand rubberBand rotateInDownRight";
					if($(".section").length>0){
						$(".animated").removeClass(window._animateName);
						var window_height = $(window).height();
						var page_num = parseInt(dest.top) / window_height;
		
						var _pBox = $(".section").eq(page_num).find(".animated"),
